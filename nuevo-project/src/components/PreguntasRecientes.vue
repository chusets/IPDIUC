<template>
    ...
    <div class="col " id="example-1">
      <div style="text-align: center; font-weight: bold;">
         <h3> Preguntas Recientes<br><br></h3>
      </div>
      <div v-for="(PregReciente, index) in PregRecientes" :key="index">
        <router-link :to="`/preguntas/${PregReciente.Num_Pregunta}`" style="text-decoration: none;">
        <v-card class="mx-auto" outlined>
          <v-card-text>
            <div class="text-body-1 font-weight-black">
              {{ PregReciente.TituloP }}
            </div>
            <div class="my-2 text-body-1">
              {{ PregReciente.Pregunt }}
            </div>
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
            <div class="my-2 text-body-1">
              Pregunta #{{ PregReciente.Num_Pregunta }} - Fecha: {{ PregReciente.Fecha_Pregunta }}
            </div>
          </v-card-actions>
        </v-card>
      </router-link>
...
<script>
  ...
  export default {
    data () {
      return {
        PregRecientes:[],n:null,i:0,dialog: false, respuesta:null, indiceP:null, preguntp:null,idpregunta:null,ResponderPArray:null,respuestap:null,ID_User_resp:null,
    ...
    mounted(){
      ...
      .then(response =>{           
            this.PregRecientes=response.data;
            this.PregRecientes.reverse();
      })  
